    ...
    <div class="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8 py-14 sm:py-18">
      <div class="grid gap-10 lg:grid-cols-12 lg:items-start">
        <div class="lg:col-span-6">
          ...
          >
            Напишите пару строк о проекте — я отвечу и предложу следующий шаг.
            Можно без деталей: достаточно цели и сроков или идеи.
          </p>
            ...
            >
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="mt-0.5 text-sm text-[color:var(--muted)]">
        ...
        <div class="lg:col-span-6">
          ...
          >
            ...
            <form class="mt-6 grid gap-4" @submit.prevent="onSubmit">
              <label class="grid gap-2">
                <span class="text-sm text-[color:var(--muted)]">Имя</span>
                <input
              ...
              <label class="grid gap-2">
                <span class="text-sm text-[color:var(--muted)]">Контакт</span>
                <input
              ...
              <label class="grid gap-2">
                <span class="text-sm text-[color:var(--muted)]">Сообщение</span>
                <textarea
              ...
              >
                <p class="text-xs text-[color:var(--muted)]"></p>

                <p
                  v-if="status === 'success'"
                  class="text-xs text-[color:var(--muted)]"
                >
                  Сообщение отправлено. Спасибо — я отвечу вам в ближайшее
                  время.
                </p>
          ...
          >
            ...
            <ul class="mt-3 grid gap-2 text-sm text-[color:var(--muted)]">
              <li class="grid grid-cols-[10px_1fr] items-start gap-3">
                <span
                  class="mt-2 h-1.5 w-1.5 rounded-full bg-[color:var(--accent)]"
                />
                <span>что вы хотите получить в итоге (цель)</span>
              </li>
              <li class="grid grid-cols-[10px_1fr] items-start gap-3">
                <span
                  class="mt-2 h-1.5 w-1.5 rounded-full bg-[color:var(--accent)]"
                />
                <span>когда нужно (дедлайн или «без спешки»)</span>
              </li>
              <li class="grid grid-cols-[10px_1fr] items-start gap-3">
                <span
                  class="mt-2 h-1.5 w-1.5 rounded-full bg-[color:var(--accent)]"
                />
                <span>что уже есть: дизайн, API, репозиторий, ТЗ</span>
              </li>
            </ul>
          </div>
        </div>
...
async function onSubmit() {
  ...
  try {
    ...
    status.value = "error";
    errorMessage.value =
      typeof e?.data?.statusMessage === "string" && e.data.statusMessage.length
